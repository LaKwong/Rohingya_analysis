...
Generated: 2026-08-05 21:11:30 PDT

## Fixes implemented

- Added a raw-first import layer under `1_data_import/fixed/` and a separated final-cleaning pipeline under `3_data_cleaning/fixed/` instead of editing the legacy scripts.
- Wrote raw-derived imports to `4_data/clean_final/imported_raw/` and final analysis datasets to `4_data/clean_final/` so existing legacy `4_data` files remain untouched.
...
- Excluded HAPEX personal PM2.5 from final outputs, following the project decision and legacy comments about damaged/less reliable monitors.
- Removed direct identifier columns from final datasets while preserving stable analysis IDs needed for joins.
- Avoided the broken legacy host pairing line by using a new host final-cleaning script.
...
- Corrected the earlier final-pipeline mistake by rebuilding final outputs from raw imports rather than relying on legacy intermediate RDS/CSV files.
...
- Applied the baseline household correction workbook and structured endline refugee survey review workbooks inside `3_data_cleaning/fixed/clean_survey_refugee_final.R`; correction audit CSVs are written under `4_data/clean_final/`.

## Resulting datasets

- `pm25_pats_refugee_ambient`:  139456 rows,   53 columns; timepoints: endline=52526; baseline=52014; midline=34916
- `pm25_pats_refugee_indoor`: 1355750 rows,   53 columns; timepoints: baseline=559683; endline=523206; midline=272861
...
- `stove_use_geocene_refugee_daily`:    2597 rows,   28 columns; timepoints: baseline=844; endline=776; (missing)=709; midline=268
...
- `survey_refugee_hh_members`:   11979 rows,   54 columns; timepoints: midline=6355; endline=5624
- `survey_refugee_household`:    3284 rows, 3150 columns; timepoints: baseline=1192; midline=1145; endline=947
- `survey_refugee_location`:    6355 rows,   41 columns; timepoints: midline=6355
- `survey_refugee_symptoms`:   11979 rows,   34 columns; timepoints: midline=6355; endline=5624
...
- Existing legacy outputs outside `4_data/clean_final/` were preserved.
