...
#Checking to make sure the data is there 
survey_data %>% 
	select(timepoint,study_arm_overall,happy) %>% 
...
# fig.width=18, fig.height=10
	survey_data %>%
	select(timepoint, study_arm_overall, fcn_id, all_of(mental_health_sig_vars)) %>%
	#drop_na() %>% 
...
# ) %>%
	...
	labs(
		...
		x =  "", # "Health category",
		y = "frequency reported by female caregivers",
		shape = "Timepoint",
		color = "Study arm"
	) + 
...
survey_data %>% 
	...
	select(timepoint,study_arm_overall, mental_health_vars) %>% 
	group_by(timepoint, study_arm_overall) %>% 
	summarise(n())
...
dind_fxn(var_interest = "suicidal_thoughts_30_yn", CES_D_data_model)



dind_fxn(var_interest = "boil_yesterday_times", model_data)
