...
fuel_used_30_days_simple_wide %>% 
  ...
  # filter(lpg == 0 & other == 0) %>% 
  # select(hh_id,starts_with("fuel_30")|starts_with("fuel_ever")) %>% view()
  ggplot(data = fuel_used_30_days_simple, 
       aes(
         x = timepoint, 
         y = prop, 
         #group = interaction(fuel_simple,study_arm_overall),
         color = fuel_simple)
       ) + 
  geom_point(size = 4) + 
  geom_errorbar(aes(x = timepoint, ymin = lower, ymax = upper, color = fuel_simple), width = 0.1) +
  scale_y_continuous(labels = scales::percent) +
  labs(
    title = "Types of Cooking Fuel Used in the Past 30 Days, By Study Arm and Timepoint",
    color = "Fuel",
...
## The following plot calculates the change of fuel use between timepoints and presente it as change % 
...
# fuel_used_30_days %>% 
#   group_by(fuel) %>% 
#   do( tidy(t.test(data=., value~timepoint)))
...
mod1 <- glm(wood ~ timepoint + study_arm_overall + timepoint * study_arm_overall, data = fuel_used_30_days_reg, family = binomial(link = "logit"))
robustse(mod1, coef = "logit")
confint(mod1)
# z test of coefficients:
# 	
# 	Estimate Std. Error  z value  Pr(>|z|)    
# (Intercept)                     -1.888047   0.132193 -14.2825 < 2.2e-16 ***
# 	timepoint.L                      0.022285   0.266493   0.0836 0.9333546    
# timepoint.Q                     -0.199820   0.183935  -1.0864 0.2773202    
# study_arm_overall.L              1.947341   0.186950  10.4164 < 2.2e-16 ***
# 	timepoint.L:study_arm_overall.L -2.819031   0.376877  -7.4800 7.434e-14 ***
# 	timepoint.Q:study_arm_overall.L  0.861559   0.260124   3.3121 0.0009259 ***
# 	---
# 	Signif. codes:  0 ‘***’ 0.001 ‘**’ 0.01 ‘*’ 0.05 ‘.’ 0.1 ‘ ’ 1
# 																	2.5 %     97.5 %
# 	(Intercept)                     -2.2050534 -1.6624899
# timepoint.L                     -0.4232080  0.6764052
# timepoint.Q                     -0.6163742  0.1344952
# study_arm_overall.L              1.6282311  2.3956228
# timepoint.L:study_arm_overall.L -3.7441971 -2.1894714
# timepoint.Q:study_arm_overall.L  0.3888808  1.4507031


mod2 <- glm(receive_lpg ~ timepoint + study_arm_overall + timepoint * study_arm_overall, data = fuel_used_30_days_reg, family = binomial(link = "logit"))
robustse(mod2, coef = "logit")
# z test of coefficients:
# 	
# 	Estimate Std. Error  z value Pr(>|z|)    
# (Intercept)                     -0.38249    0.18515  -2.0658  0.03884 *  
# 	timepoint.L                      9.59225    0.29993  31.9815  < 2e-16 ***
# 	timepoint.Q                     -4.19899    0.34017 -12.3439  < 2e-16 ***
# 	study_arm_overall.L             -6.09074    0.26185 -23.2604  < 2e-16 ***
# 	timepoint.L:study_arm_overall.L 10.42742    0.42419  24.5817  < 2e-16 ***
# 	timepoint.Q:study_arm_overall.L -4.03947    0.48108  -8.3967  < 2e-16 ***
# 	---
# 	Signif. codes:  0 ‘***’ 0.001 ‘**’ 0.01 ‘*’ 0.05 ‘.’ 0.1 ‘ ’ 1
# confint(mod2)
# 																	2.5 %    97.5 %
# 	(Intercept)                     -33.54566 -22.26010
# timepoint.L                      56.00163  79.94188
# timepoint.Q                     -44.81555 -30.99357
# study_arm_overall.L             -52.99054 -37.03035
# timepoint.L:study_arm_overall.L  76.06020 109.91683
# timepoint.Q:study_arm_overall.L -61.47996 -41.93273

# stargazer(robustse(mod2, coef = "logit")) # produces tables that economists use
...
survey_data %>% 
	...
	count(study_arm_overall, timepoint, days_from_refill)
