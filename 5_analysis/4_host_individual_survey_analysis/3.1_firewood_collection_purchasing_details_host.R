...
### Looking at changes due to intervention
dind_fxn(c("fuel_30_buy_wood", "fuel_30_gather_scraps", "fuel_30_collect_wood"), model_data)

model_data %>% group_by(timepoint, study_arm_overall) %>% 
	summarize(sum(fuel_30_collect_wood))

dind_fxn(c("radon"), model_data)
...
# For each baseline, midline, endline there are 437 comparison hh and 496 intervention hh
# 933 hh with triplicate measures
...
#	count(study_arm_overall, timepoint, vars, value) %>%
	count(vars, value) %>% 
	filter(value == 1) %>% 
...
#	mutate(prop = prop.table(n) * 100) %>%
	...
	pivot_wider(names_from = value, values_from = c(n)) %>%
	#select(-c(n_0)) %>%
	#replace_na(list(n_1 = 0L, prop_1 = 0)) %>% # need 0L because n_1 is an integer
	pivot_wider(names_from = c("sex"), values_from = c("1"), names_vary = "slowest") %>% # slowest" makes it vary first by the values then the names
	mutate(
		fuel_procurement = 
			ordered(
				vars,
