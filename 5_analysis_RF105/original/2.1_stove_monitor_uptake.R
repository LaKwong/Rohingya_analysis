...
# @Description: Stove use data analysis. This file compiles data collected from geoscene monitors
   #The first 250 rows are data cleaning steps --
# @Version: 3.6.2
# @Date: 240816
...
file_events_end <- here::here("2_data_raw/Geocene_240209/events.csv")
file_mission_logs_end <- here::here("2_data_raw/Geocene_240209/mission_logs.csv")
file_missions_end <- here::here("2_data_raw/Geocene_240209/missions.csv")
file_sensors_end <- here::here("2_data_raw/Geocene_240209/sensors.csv")
file_tags_end <- here::here("2_data_raw/Geocene_240209/tags.csv")
...
#Read in the survey data to compare to the stove use data 
survey_data <- 
...
df_events_stove_on %>% 
  group_by(study_arm_overall, fcn_id, lpg_enrolled_and_receiving) %>% 
  distinct(date) %>% 
  ungroup()
...
#Restricts data to just those within 3 months of start ## This is the analyiss dataset that is used 
...
df_events_stove_on_per_day <- #This calculates a daily number of stove uses per household per date 
  df_events_stove_on_lt_3mo %>%
 filter(fcn_id %notin% c(df_have_geocene_but_no_survey_data_fcn_id)) %>% ## Conversation with Layla do not restrict based on this 
  ...
  summarise(
    n = n(), #Gets renamed later 
    stove_on_min_sum = sum(stove_on_min)
    ) %>% 
  ...
  mutate(
    ...
    stove_on_min_sum_biomass_na = ifelse(!is.na(stove_on_min_sum_biomass), as.numeric(as.character(stove_on_min_sum_biomass)), NA),
    stove_on_min_sum_lpg_na = ifelse(!is.na(stove_on_min_sum_lpg), as.numeric(as.character(stove_on_min_sum_lpg)), NA), 
    timepoint = case_when(date < ymd("2020-04-01") ~ "baseline",
...
df_events_stove_on_per_day %>% 
  group_by(lpg_enrolled_and_receiving, study_arm_overall, timepoint) %>% 
...
df_events_stove_on_per_day %>% 
  group_by(lpg_enrolled_and_receiving) %>% 
  count(fcn_id) ## The numbr of independant households monitored 

df_events_stove_on_per_day %>% 
  mutate(timeline = case_when(date < ymd("2020-05-01") ~ "baseline",
                              year(date) == 2022 ~ "endline",
                              .default = "midline")) %>% 
  group_by(lpg_enrolled_and_receiving) %>% 
  ...
  summarise(
   ...
   # count(unique(date)),
    ...
    mean(cooking_events_with_biomass, na.rm = TRUE),
    sd(cooking_events_with_biomass, na.rm = TRUE),
    mean(cooking_events_with_lpg, na.rm = TRUE),
    sd(cooking_events_with_lpg, na.rm = TRUE),
...
#	filter(fcn_id %notin% c(df_have_geocene_but_no_survey_data_fcn_id)) %>%
	...
	summarise(
	  n = n (), 
	  day_exclusive_lpg = sum(exclusive_lpg, na.rm = TRUE)) %>%
	arrange(n) #6891/7637
...
fig_stove_monitored_per_day <-
  df_stove_monitored_per_day %>%
	#left_join(df_stove_monitored_per_day, by = c("fuel_type","days_after_first_receiving")) %>%
	# filter(lpg_enrolled_and_receiving == "receiving LPG through distribution program") %>%
