...
#   results produced by 8_geocene_stove_use_combined_reviewed.R. This script is
...
#
# Inputs from 8_geocene_stove_use_combined_reviewed.R:
#   7_tables/RF105_reviewed_YYYYMMDD/geocene_stove_use_daily_analysis_dataset_reviewed.csv
#   7_tables/RF105_reviewed_YYYYMMDD/geocene_stove_use_daily_summary_reviewed.csv
#
# Outputs:
...
read_geocene_reviewed_csv <- function(filename) {
  ...
  if (!file.exists(file_path)) {
    stop(
      ...
      "Run 5_analysis_RF105/reviewed/8_geocene_stove_use_combined_reviewed.R first.",
      call. = FALSE
    )
  }
...
geocene_daily_file <- "geocene_stove_use_daily_analysis_dataset_reviewed.csv"
geocene_daily_summary_file <- "geocene_stove_use_daily_summary_reviewed.csv"
...
stove_result_source_dependency <- tibble(
  downstream_script = "2.1_stove_monitor_uptake_reviewed.R",
  source_script = "8_geocene_stove_use_combined_reviewed.R",
...
write_reviewed_csv(
  ...
  "stove_geocene_result_source_dependency_reviewed.csv",
  subfolder = "qa"
)
...
write_reviewed_csv(
  flag_missing_vars(
    stove_data_raw,
    stove_required_vars,
    "reviewed Geocene daily analysis dataset from 8_geocene_stove_use_combined_reviewed.R"
  ),
