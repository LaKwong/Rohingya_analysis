...
################################################################################
# Refugee household-member, symptom, and location subforms
#
# These related survey outputs are now cleaned in this consolidated script.
# They are kept here so the refugee survey cleaning workflow has one maintained
...
clean_refugee_related_survey <- function(role, dataset_name, output_file) {
  ...
  household_raw <- read_rds_required("4_data/clean_final/imported_raw/survey_refugee_household_raw.rds")
  excluded_parent_keys <- character()
  if (all(c("KEY", "study_arm_overall") %in% names(household_raw))) {
    excluded_parent_keys <- household_raw$KEY[household_raw$study_arm_overall %in% c("host", "host_in_refugee_form")]
    excluded_parent_keys <- excluded_parent_keys[!is.na(excluded_parent_keys)]
  }
  if ("PARENT_KEY" %in% names(data) && length(excluded_parent_keys)) {
    data <- data[!(data$PARENT_KEY %in% excluded_parent_keys), , drop = FALSE]
  }

  household_exclusion_audit <- clean_final_path(
    "4_data",
    "clean_final",
    "survey_refugee_household_exclusion_audit.csv"
  )
  removed_excluded_household_children <- 0L
  if ("PARENT_KEY" %in% names(data) && file.exists(household_exclusion_audit)) {
    excluded_household_rows <- read.csv(
      household_exclusion_audit,
      stringsAsFactors = FALSE,
      check.names = FALSE
    )
    if ("KEY" %in% names(excluded_household_rows)) {
      excluded_household_keys <- excluded_household_rows$KEY[!is.na(excluded_household_rows$KEY)]
      excluded_household_child_idx <- which(data$PARENT_KEY %in% excluded_household_keys)
      removed_excluded_household_children <- length(excluded_household_child_idx)
      if (removed_excluded_household_children) {
        data <- data[-excluded_household_child_idx, , drop = FALSE]
      }
    }
  }
  ...
  entry <- make_inventory_entry(
    ...
    shareable_removed_identifier_columns = shareable$removed,
    notes = paste(
      "Refugee", role, "survey subform rebuilt from raw-first imports.",
      paste0(
...
clean_refugee_related_survey("hh_members", "survey_refugee_hh_members", "survey_refugee_hh_members.rds")
clean_refugee_related_survey("symptoms", "survey_refugee_symptoms", "survey_refugee_symptoms.rds")
clean_refugee_related_survey("location", "survey_refugee_location", "survey_refugee_location.rds")
