...
# file_survey_data_host_base <- here::here("4_data/RohingyaFuel_survey_data_long_paired_host.rds")




# files out
file_out_survey_data_1 <- here::here("4_data/RohingyaFuelMaster_survey_data_1.rds")

file_out_tidy_data_long <- here::here("4_data/RohingyaFuelMaster_tidy_long.rds")
file_out_survey_data_intervention_baseline <- here::here("4_data/RohingyaFuelMaster_survey_data_intervention_baseline.rds")
file_out_survey_data_intervention_endline <- here::here("4_data/RohingyaFuelMaster_survey_data_intervention_endline.rds")
file_out_survey_data_control_baseline <- here::here("4_data/RohingyaFuelMaster_control_baseline.rds")
file_out_survey_data_control_endline <- here::here("4_data/RohingyaFuelMaster_control_endline.rds")
file_out_survey_data_intervention <- here::here("4_data/RohingyaFuelMaster_intervention.rds")
file_out_survey_data_control <- here::here("4_data/RohingyaFuelMaster_control.rds")
file_out_tidy_data_wide <- here::here("4_data/RohingyaFuelMaster_tidy_wide.rds")

file_out_hh_data <- here::here("4_data/RohingyaFuelMaster_hh_data.xlsx")

file_out_tidy_data_host_long <- here::here("4_data/RohingyaFuelMaster_tidy_host_long.rds")
file_out_hh_data_host <- here::here("4_data/RohingyaFuelMaster_hh_data_host.xlsx")

# file_out <- here::here("4_data/data.rds")


# load files
survey_data_base <- read_rds(file_survey_data_base)
write.csv(survey_data_base, here::here("4_data/RohingyaFuel_survey_data_long.csv"))

# survey_data_host_base <- read_rds(file_survey_data_host_base)
# ============================================================================
...
# Function to coalese
survey_data_1_clean_fcn <- function(df){
	df %>%
		select(
		...
		mutate(
			...
			not_eat1 = coalesce(as.numeric(not_eat1), as.numeric(not_eat.1), as.numeric(reduce_meals_1)),
			...
			time_child_gathering_nonwood_items = coalesce(time_child_gathering_nonwood_ite, time_child_gathering_non.wood_items, `time_child_gathering_non-wood_items`),
...
survey_data_1 <- 
	survey_data_1_clean_fcn(survey_data_base)

# survey_data_1_host <- 
# 	survey_data_1_clean_fcn(survey_data_host_base)


# Function to set NAs and ordered values
survey_data_2_clean_fcn <-
	function(df){
		df %>%
			mutate(
...
survey_data_2 <-
	survey_data_2_clean_fcn(survey_data_1)

# 1127 failed to parse in two columns, not sure which two!
	
survey_data_long <- survey_data_2

# survey_data_host_long <- survey_data_2_host
...
survey_data_control_baseline <-
	survey_data_long %>%
	filter(study_arm == "intervention")

survey_data_control_endline <-
	survey_data_long %>%
...
survey_data_control <-
	survey_data_control_baseline %>%
	left_join(survey_data_control_endline, by = c("fcn_id"), suffix = c(".baseline", ".endline"))

survey_data_wide <-
	bind_rows(survey_data_intervention, survey_data_control) %>%
	mutate(
		study_arm =
			case_when(
...
write_rds(survey_data_1, file_out_survey_data_1)

write_rds(survey_data_long, file_out_tidy_data_long)
...
write_rds(survey_data_control_baseline, file_out_survey_data_control_baseline)
write_rds(survey_data_control_endline, file_out_survey_data_control_endline)
write_rds(survey_data_intervention, file_out_survey_data_intervention)
write_rds(survey_data_control, file_out_survey_data_control)
write_rds(survey_data_wide, file_out_tidy_data_wide)

# write_rds(survey_data_host_long, file_out_tidy_data_host_long)

# openxlsx::write.xlsx(hh_data, file_out_hh_data)
# openxlsx::write.xlsx(hh_data_host, file_out_hh_data_host)
