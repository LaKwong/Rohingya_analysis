...
# check if the var needs to be coalesced or not
# survey_data_base %>% select(starts_with("assets_valued"))

## select multiple in v117

## not in prior surveys - all of these are var.0 (and don't have var0)
# books_where 
# engage_books 
# engage_stories
# engage_songs
...
# engage_play
# engage_count
# food_consump_adult_day

##### already in prior surveys ####
# rice_source
# bread_source
# corn_source
# potatoes_source
...
## not in prior surveys - all of these are var.0 (and don't have var0)
# assets_valued # assets_valued,0
# assets_desired
# income_abroad_relative
# income_abroad_location
# livlihood_training
# livlihood_skills_not
# respondent_pregnancy_location_1 (#1 refers to the first pregnancy)
# respondent_pregnancy_assistance_1
# respondent_birth_assistance_1
# respondent_pregnancy_decision_1
# respondent_pregnancy_location_2 (#2 refers to the first pregnancy)
# respondent_pregnancy_assistance_2
# respondent_birth_assistance_2
# respondent_pregnancy_decision_2
# respondent_pregnancy_location_3 (#3 refers to the first pregnancy)
# respondent_pregnancy_assistance_3
# respondent_birth_assistance_3
# respondent_pregnancy_decision_3
# lpg_stove_repair
# lpg_stove_repair_inspect
# lpg_cylinder_repair
# lpg_cylinder_repair_inspect
# lpg_repair_details
# heat_control
# cool_control
# burn_plastic_types
# burn_plastic_reasons
# trad_stove_use_fuel
# food_flavor
# food_burn_consequence
# fuel_type_nonLPG
# lpg_changes_lifestyle
# nursery_resp
# nursery_own
# neg_exp_firewood_men
# neg_exp_firewood_women
# neg_exp_firewood_boy
# neg_exp_firewood_girls
# neg_exp_LPG_men
# neg_exp_LPG_women
# neg_exp_LPG_boy
# neg_exp_LPG_girls
...
## hh-location
# location_outside_house_1 (not sure why we put a 1 on the end of this var but I think it meant "yesterday")


## hh-symptoms
# symptoms_7 (7 stands for "in the past 7 days")


# There are many vars that have <var>1 and <var>.1 because of a change in the survey xlm that happened after hh 9I100788I3_5992, the last pre-intervention hh, which was in camp 9 and surveyed 10/2. The first intervention hh, 4E184565Pp21_514, was in camp 4 and surveyed 10/10. This change happened when we 
...
survey_data_coalesce_fcn <- function(df){
	df %>%
		...
		mutate(first_enrolled_lpg_ymd = dmy(first_enrolled_lpg)) %>%
		mutate(first_receive_lpg_ymd = dmy(first_receive_lpg)) %>%
		mutate(first_wait_lpg = as.numeric(difftime(first_enrolled_lpg_ymd, first_receive_lpg_ymd, unit = "days"))) %>%
		mutate(collect_wood_duration = as.numeric(difftime(dmy(collect_wood_forest_stop), dmy(collect_wood_forest_start), unit = "days"))) %>%
		mutate_at(
			vars(
				enumerator_name,
		...
		mutate_at(
			vars(
				traditional_use_yesterday, # Number of times traditional stove was used yesterday, among hh that reported using anything but LPG (this is not very helpful, better to make all NAs zero so it is among all hh)
				LPG_use_yesterday, # Number of times LPG stove was used yesterday, among hh that reported receiving or purchasing LPG (this is not very helpful, better to make all NAs zero so it is among all hh)

				boil_yesterday_times, # Number of times the stove was used for boiling water yesterday, of the hh that said they used the stove to boil something to drink (this is not very helpful, better to make all NAs zero so it is among all hh)
		...
		# see reasoning directly above
		mutate(traditional_use_yesterday = ifelse(is.na(traditional_use_yesterday), 0, traditional_use_yesterday)) %>%
		mutate(LPG_use_yesterday = ifelse(is.na(LPG_use_yesterday), 0, LPG_use_yesterday)) %>%
		mutate(boil_yesterday_times = ifelse(is.na(boil_yesterday_times), 0, boil_yesterday_times)) %>%
		...
		rename( 
			...
			respondent_resp_rate  = resp_rate_reported_respondent, #removed 10/4/22 becasue they were breaking the script
			respondent_weight_loss = weight_loss_reported_respondent,
...
survey_data_set_nas_order_fcn <-
	function(df){
		df %>%
			mutate(
				timepoint_num = ifelse(timepoint == "baseline", 0, ifelse(timepoint == "endline", 1, 0.5)), # endline == 1
		...
		mutate_at(
			...
			mutate(
				collect_wood_forest_start_ymd = 
					case_when(
						# convert to yyyy-mm-dd
						timepoint == "baseline" ~ dmy(collect_wood_forest_start),
						timepoint == "midline" ~ mdy(collect_wood_forest_start),
						timepoint == "endline" ~ mdy(collect_wood_forest_start) # didn't ask this question at endline? but we did ask how long firewood collection took....
					),
				collect_wood_forest_stop_ymd = 
					case_when(
						timepoint == "baseline" ~ dmy(collect_wood_forest_stop),
						timepoint == "midline" ~ mdy(collect_wood_forest_stop),
						timepoint == "endline" ~ mdy(collect_wood_forest_stop) # didn't ask this question at endline? but we did ask how long firewood collection took....
					),
				collect_wood_duration = difftime(collect_wood_forest_stop_ymd, collect_wood_forest_start_ymd, unit = "days"),
				collect_wood_duration = as.numeric(collect_wood_duration)
			) %>% 
		...
		# ## Chris added this, shouldn't be necessary because imported with read_csv(name_repair = "universal")
...
## Chris added this, which is unnecessary
## Edited 7/17/22
...
# 
# 
# #Replace all / with . in the dataset (the reasons Layla didn't do this is because it would have resulted in mutliple columns with the same name, which probably would've broken the code)
...
survey_data_clean <-
	survey_data_set_nas_order_fcn(survey_data_coalesced_cols) # %>% select(-c(starts_with("repsondent_pregnancy"))))
...
# survey_data_host_clean %>%
# 	filter(study_arm == 1) %>%
# 	select(hh_id, name_respondent, name_hh_head, target_child_name)

# hh_id                                   name_respondent name_hh_head target_child_name
# <chr>                                   <chr>           <chr>        <chr>            
# 	1 Fathe ali para1109_2219047599800_ T3250 Ismot Ara       Md Hossen    Zahangir Alam  

# survey_data_host_clean %>%
# 	filter(name_hh_head == "Md Hossen") %>% # target_child_name == "Zahangir Alam", name_respondent == "Ismot Ara"
# 	select(study_arm, hh_id, name_respondent, name_hh_head, target_child_name)
...
# Write
###############################################################################
write_rds(survey_data_clean, file_out_1)
